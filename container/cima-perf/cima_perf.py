...
def _test_throughput(svc_call):
    ...
    for _ in range(task_num):
        # 2. Each process invokes the CIMA API (either via SDK or service directly) repeatedly
...
def _test_response(svc_call):
    ...
    for _ in range(repeat_times):
        ...
        #   Begin timing.
        #   Call (one immediately after another) the CIMA API (either via SDK or
...
def _sdk_get_cc_measurement():
    """Using CIMA SDK to get CC measurement."""
    # Current just test the first IMR with index 0 and hash algorithm ID 12.
    CimaSdk.inst().get_cc_measurement([0, 12])

def _sdk_get_cc_eventlog():
    """Using CIMA SDK to get CC eventlog."""
    CimaSdk.inst().get_cc_eventlog()

def _sdk_get_cc_report():
    """Using CIMA SDK to get CC report (i.e. quote)."""
    CimaSdk.inst().get_cc_report()

def test_svc_get_cc_measurement_throughput():
    """Test the throughput of CIMA Service get_cc_measurement."""
    _test_throughput(_sdk_get_cc_measurement)

def test_svc_get_cc_measurement_response():
    """Test the response time of CIMA Service get_cc_measurement."""
    _test_response(_sdk_get_cc_measurement)

def test_svc_get_cc_eventlog_throughput():
    """Test the throughput of CIMA Service get_cc_eventlog."""
    _test_throughput(_sdk_get_cc_eventlog)

def test_svc_get_cc_eventlog_response():
    """Test the response time of CIMA Service get_cc_eventlog."""
    _test_response(_sdk_get_cc_eventlog)

def test_svc_get_cc_report_throughput():
    """Test the throughput of CIMA Service get_cc_report."""
    _test_throughput(_sdk_get_cc_report)

def test_svc_get_cc_report_response():
    """Test the response time of CIMA Service get_cc_report."""
    _test_response(_sdk_get_cc_report)

def test_cima_init():
    """Test the initialization time of CIMA.
    i.e. The time cost of the initialization for CIMA Device Plugin and CIMA Service so
    ...
    #   Begin timing.
    #   Start CIMA deployment (incl. CIMA Webhook and CIMA Service).
    #   Polling the readiness of CIMA service until it's ready.
    #   End timing.
