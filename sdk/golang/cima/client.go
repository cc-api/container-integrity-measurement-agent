...
const (
	UDS_PATH = "unix:/run/cima/uds/cima-server.sock"
)

type Client struct {
	client pb.CimaClient
}

func NewClient() (Client, error) {
	conn, err := grpc.Dial(UDS_PATH, grpc.WithInsecure())
	if err != nil {
		log.Fatalf("[GetCCReportFromServer] can not connect to CIMA server UDS at %v with error: %v", UDS_PATH, err)
		return Client{}, err
	}

	client := pb.NewCimaClient(conn)
